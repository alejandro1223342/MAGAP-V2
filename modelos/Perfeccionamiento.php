...
class Perfeccionamiento
{
    ...
    {
        $sql = "call sp_perfeccionamiento ('list', 0, 0, 0, 0, 0)";
        return ejecutarConsultaSP($sql);
    }

    public function mostrar($tra_id)
    {
        $sql = "call sp_perfeccionamiento('mos','$tra_id',0,0,0,0)";
        $result = ejecutarConsultaSP($sql);
        return $result->fetch_assoc();
    }
    public function tabla($id)
    {
        $sql = "call sp_perfeccionamiento('listedi',0,0,'$id',0,0)";
    ...
    {
        // Ejecutar la consulta y obtener el resultado
        $sql = "CALL sp_procesos('perfp', 0, $usu_id, $tra_id, $cat_id_estado, '$pro_observacion')";
        return ejecutarConsulta($sql);
    }
