...
function driveGestores($parentFolderId, $folderName, $fileName, $fileTempPath,$proceso, $usu_nombre)
{
    ...
    try {
        ...
        $usuFolderID = null;
        $optParams = [
            'q' => "mimeType='application/vnd.google-apps.folder' and name='$usu_nombre' and '$monthFolderId' in parents",
            'spaces' => 'drive',
        ];
        $results = $service->files->listFiles($optParams);
        ...
        if (count($results->getFiles()) == 0) {
            ...
            $usuFolder = $service->files->create($fileMetadata, ['fields' => 'id']);
            $usuFolderID = $usuFolder->id;
        } else {
            $usuFolderID= $results->getFiles()[0]->getId();
        }

        // Verificar si la última subcarpeta existe en la carpeta del mes actual
        $lastFolderId = null;
        $optParams = [
            'q' => "mimeType='application/vnd.google-apps.folder' and name='$folderName' and '$usuFolderID' in parents",
            'spaces' => 'drive',
        ];
        $results = $service->files->listFiles($optParams);
