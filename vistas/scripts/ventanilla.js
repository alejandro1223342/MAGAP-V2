...
function cargarDatosGuardados(s_ident) {
    ...
    if (sol_identificacion) {
        $('#tabla_pdf tbody tr').each((index, element) => {
            ...
            if (cambios) {
                ...
                if (estado === '18') {
                    $(element).find('td:last').html('<span class="text-success">Registrado</span>');
                } else {
                    $(element).find('td:last').html('<button class="btn btn-editar btn-primary btn-xs">Cambiar<i class="fa fa-pen" style="margin-left: 5px;"></i></button>');
                }

                $(element).find('td:eq(5)').html('<input type="text" value="' + datos.pro_observacion + '" readonly class="form-control">');
            }
        });
    }
    $('body').on('click', '.btn-editar', function (event) {
        ...
        fila.find('td:last').append('<div class="btn-group" role="group" aria-label="Botones de acción"><button class="btn btn-aceptar btn-success btn-xs">Aceptar <i class="fa fa-save" style="margin-left: 5px;"></i></button><button class="btn btn-cancelar btn-danger btn-xs">Cancelar</button></div>');
    });
    ...
    $('body').off('click', '.btn-aceptar').on('click', '.btn-aceptar', function (event) {
        event.preventDefault();
        guardar();
        let fila = $(this).closest('tr');
